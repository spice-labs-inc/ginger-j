...
```
ginger-j/<qualified suite or class>#<test name>     code tests
ginger-j/<data path>[#<case id>]                    data-driven cases: the file's `id` field
```

Surveyor's `scripts/compare-test-ids.py` diffs those IDs against `tests/coverage/ginger-j.tsv`
...
- Nothing here is observable through `spice --no-upload`; the upload protocol tests stay
  unit-only (mock server). The jar records its commit in `META-INF/git/ginger-j.properties`.
