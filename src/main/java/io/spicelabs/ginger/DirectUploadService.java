...
public class DirectUploadService {
    ...
    public static void uploadDirect(
        ...
        log.info("Init response: presignedUrl={}, bundleId={}",
                initResponse.presignedUrl().substring(0, Math.min(50, initResponse.presignedUrl().length())) + "...",
                initResponse.bundleId());
     ...
     */
    private static InitResponse initUpload(
            String baseUrl,
            String jwt,
            String publicKeyPem,
            String sha256,
        ...
        try (Response response = executeWithRetry(requestSupplier, "Init upload")) {
            if (!response.isSuccessful()) {
                ...
                throw new IOException("Failed to initialize direct upload: " + response.code() + " " + errorBody);
            }
            ...
            if (presignedUrlNode == null || uploadTokenNode == null || bundleIdNode == null) {
                StringBuilder missing = new StringBuilder();
                if (presignedUrlNode == null) missing.append("presignedUrl, ");
                if (uploadTokenNode == null) missing.append("uploadToken, ");
                if (bundleIdNode == null) missing.append("bundleId, ");
                if (missing.length() > 2) missing.setLength(missing.length() - 2);
                throw new IOException("Init response missing required field(s): " + missing);
            }
     ...
     */
    private static void uploadToStorage(String presignedUrl, File bundle) throws IOException {
     ...
     */
    private static CompleteResponse completeUpload(
            String baseUrl,
            String jwt,
            String uploadToken,
            String sha256
        ...
        try (Response response = executeWithRetry(requestSupplier, "Complete upload")) {
            if (!response.isSuccessful()) {
                String errorBody = response.body() != null ? response.body().string() : "";
                throw new IOException("Failed to complete direct upload: " + response.code() + " " + errorBody);
            }
     ...
     */
    private static Response executeWithRetry(Supplier<Request> requestSupplier, String operationName) throws IOException {
        IOException lastException = null;
        long backoffMs = INITIAL_BACKOFF_MS;

        for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                Response response = CLIENT.newCall(requestSupplier.get()).execute();
                ...
                // Server error (5xx) - close response and retry
                String body = response.body() != null ? response.body().string() : "";
                response.close();

                if (attempt < MAX_RETRIES) {
                    log.warn("{} failed with {} (attempt {}/{}), retrying in {}ms: {}",
                            operationName, response.code(), attempt, MAX_RETRIES, backoffMs, body);
